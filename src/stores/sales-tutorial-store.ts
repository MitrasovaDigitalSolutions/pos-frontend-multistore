...
import type {
    SalesTutorialId,
    SalesTutorialPreSnapshot,
} from "@/features/sales-tutorial/types/sales-tutorial";
...
interface SalesTutorialStoreState {
    activeTutorial: SalesTutorialId | null;
    stepIndex: number;
    isRunning: boolean;
    isMenuOpen: boolean;
    preSnapshot: SalesTutorialPreSnapshot | null;
    cursor: CursorState;

    // Actions
    startTutorial: (id: SalesTutorialId) => void;
    stopTutorial: () => void;
    setStepIndex: (index: number) => void;
    nextStep: () => void;
    prevStep: () => void;
    setMenuOpen: (open: boolean) => void;
    saveSnapshot: (snapshot: SalesTutorialPreSnapshot) => void;
    clearSnapshot: () => void;
    updateCursor: (cursor: Partial<CursorState>) => void;
    triggerCursorClick: () => void;
}

export const useSalesTutorialStore = create<SalesTutorialStoreState>((set) => ({
    activeTutorial: null,
    stepIndex: 0,
    isRunning: false,
    isMenuOpen: false,
