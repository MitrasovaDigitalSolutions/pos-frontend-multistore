...
import type { AuditTutorialId } from "@/features/audit/tutorial/types/audit-tutorial";
...
interface AuditTutorialStoreState {
    activeTutorial: AuditTutorialId | null;
    stepIndex: number;
    isRunning: boolean;
    isMenuOpen: boolean;
    cursor: CursorState;

    startTutorial: (id: AuditTutorialId) => void;
    stopTutorial: () => void;
...
export const useAuditTutorialStore = create<AuditTutorialStoreState>((set) => ({
    activeTutorial: null,
    stepIndex: 0,
    isRunning: false,
    isMenuOpen: false,
    ...
    updateCursor: (patch) =>
        set((state) => ({ cursor: { ...state.cursor, ...patch } })),
