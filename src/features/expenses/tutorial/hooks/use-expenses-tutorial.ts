...
import { EXPENSES_TUTORIAL_STEPS } from "../steps";
...
function resolveStepTargetSelector(step: ExpensesTutorialStep): string {
...
export function useExpensesTutorial() {
    const activeTutorial = useExpensesTutorialStore((state) => state.activeTutorial);
    const stepIndex = useExpensesTutorialStore((state) => state.stepIndex);
    const isRunning = useExpensesTutorialStore((state) => state.isRunning);

    const rawStartTutorial = useExpensesTutorialStore((state) => state.startTutorial);
    const stopTutorial = useExpensesTutorialStore((state) => state.stopTutorial);
    const setStepIndex = useExpensesTutorialStore((state) => state.setStepIndex);
    const updateCursor = useExpensesTutorialStore((state) => state.updateCursor);
    const triggerCursorClick = useExpensesTutorialStore((state) => state.triggerCursorClick);
    ...
    const tutorialSteps: ExpensesTutorialStep[] = useMemo(() => {
        if (!activeTutorial) return [];
        return EXPENSES_TUTORIAL_STEPS[activeTutorial] || [];
    }, [activeTutorial]);
    ...
    const joyrideSteps: Step[] = useMemo(() => {
        ...
        return tutorialSteps.map((s) => {
            ...
            return {
                ...
                // yang menunda mount card kustom ~500ms lebih lambat dari panah.
                target: activeTarget,
                title: s.title,
                content: s.content,
                placement:
