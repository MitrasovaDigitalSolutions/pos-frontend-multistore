...
export const paymentSchema = z.object({
    receiving_id: z.preprocess(
        ...
        z.number({ message: "Penerimaan barang wajib dipilih" }).min(1, "Penerimaan barang wajib dipilih")
    ),
    ...
    cash_account_id: z.preprocess(
        (val) => {
            if (val === "" || val === undefined || val === null) return undefined;
            const num = Number(val);
            return isNaN(num) ? undefined : num;
        },
        z.number({ message: "Rekening/akun kas wajib dipilih" }).min(1, "Rekening/akun kas wajib dipilih")
    ),
