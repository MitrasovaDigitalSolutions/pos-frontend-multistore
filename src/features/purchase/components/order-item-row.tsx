...
export function OrderItemRow({
    ...
    const { watch, setValue, getValues } = useFormContext<PurchaseOrderInput>();

    const productId = watch(`items.${idx}.product_id`);
    const selectedProduct = products.find((p) => p.id === Number(productId));

    const currentQty = watch(`items.${idx}.kuantitas`) || 0;
    const currentPrice = watch(`items.${idx}.harga_estimasi`) || 0;
            ...
            <div className="grid grid-cols-12 gap-3 items-end">
                ...
                <div className="col-span-12 sm:col-span-5">
                    <FormSelect<PurchaseOrderInput>
                        name={`items.${idx}.product_id` as FieldPath<PurchaseOrderInput>}
                        ...
                        onChange={(val) => {
                            ...
                            if (prod) {
                                setValue(`items.${idx}.harga_estimasi` as FieldPath<PurchaseOrderInput>, prod.harga_beli ?? 0);

                                // Default qty to 1 if empty/0
                                const currentQty = getValues(`items.${idx}.kuantitas` as FieldPath<PurchaseOrderInput>);
                                if (!currentQty) {
                                    setValue(`items.${idx}.kuantitas` as FieldPath<PurchaseOrderInput>, 1);
                                }
                            }
                        }}
                    />
                ...
                <div className="col-span-4 sm:col-span-2">
                    <FormNumberInput<PurchaseOrderInput>
                        name={`items.${idx}.kuantitas` as FieldPath<PurchaseOrderInput>}
                        placeholder="Qty"
                        disabled={isPending}
                        label="Qty"
                    />
                ...
                <div className="col-span-8 sm:col-span-4">
                    <FormNominalInput<PurchaseOrderInput>
                        name={`items.${idx}.harga_estimasi` as FieldPath<PurchaseOrderInput>}
                        placeholder="Rp 0"
                        disabled={isPending}
            ...
            {selectedProduct && (
                <div className="pt-3 border-t border-slate-100 flex justify-between items-center text-xs">
                    ...
                    <div className="flex items-center gap-1.5">
                        <span className="text-slate-400">Subtotal Estimasi:</span>
