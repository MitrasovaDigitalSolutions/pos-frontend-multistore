...
export interface ConsignmentPriceAlertFormInput {
  items: {
    product_uid: string;
...
interface ConsignmentPriceAlertDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  priceAlerts: PriceComparisonItem[];
  isFinalizing: boolean;
  onCompleteWithoutPrices: () => void;
  onCompleteWithPrices: (formValues: ConsignmentPriceAlertFormInput) => void;
}

export function ConsignmentPriceAlertDialog({
  open,
  onOpenChange,
  priceAlerts,
  isFinalizing,
...
}: ConsignmentPriceAlertDialogProps) {
  const adjustmentMethod = useSettingsStore(
    (s) => s.settings["hpp_adjustment_method"] ?? "latest"
  );
  const buyPriceBasis = (alert: PriceComparisonItem) =>
    adjustmentMethod === "average" ? alert.harga_beli_avg : alert.harga_beli_baru;

  const alertFormMethods = useForm<ConsignmentPriceAlertFormInput>({
    defaultValues: {
      items: [],
    },
  });
  ...
  useEffect(() => {
    if (open && priceAlerts.length > 0) {
      const initialItems = priceAlerts.map((alert) => {
        ...
        const calculatedHargaJualSaran = Math.round(
    ...
    formItems.forEach((item, idx) => {
      ...
      if (activeId === `items.${idx}.margin_baru`) {
        if (item.margin_baru !== prev.margin_baru) {
          const calculatedHargaJual = Math.round(
    ...
    <BaseDialog
      ...
      title={
        <span className="flex items-center gap-2">
          ...
          <span>Peringatan Perubahan Harga Beli Konsinyasi</span>
        </span>
      }
      scrollable={false}
      ...
      <FormProvider {...alertFormMethods}>
        <div className="space-y-4 my-4">
          <p className="text-xs text-slate-500 leading-relaxed">
            Sistem mendeteksi adanya perubahan harga beli dari supplier dibandingkan dengan harga beli master. Silakan tinjau perubahan berikut dan Anda dapat memperbarui harga jual atau margin produk secara langsung:
          </p>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                  ...
                  {adjustmentMethod === "average" && (
                    <th className="p-3 text-right text-indigo-600 dark:text-indigo-400">
                      Harga Beli Average
                    </th>
                  )}
              ...
              <tbody className="divide-y divide-slate-50 font-medium">
                {priceAlerts.map((alert, idx) => {
                  ...
                  const buyPriceNew = buyPriceBasis(alert);
                  const calculatedHargaJualSaran =
                    adjustmentMethod === "average" && alert.harga_jual_saran_avg
                    ...
                    <tr key={alert.product_uid} className="hover:bg-slate-50/50">
                      ...
                      {adjustmentMethod === "average" && (
                        <td className="p-3 text-right whitespace-nowrap">
                          ...
                          <div className="text-[10px] text-indigo-500/80 font-medium">
                            Rata-rata
                          </div>
                        </td>
                      )}
                          ...
                          <div className="space-y-1.5 animate-fade-in">
                            <div className="flex gap-2 items-center">
                              <div className="space-y-0.5">
                                ...
                                <FormNumberInput<ConsignmentPriceAlertFormInput>
                              ...
                              <div className="space-y-0.5">
                                ...
                                <FormNominalInput<ConsignmentPriceAlertFormInput>
                            ...
                            <div className="text-[10px] text-slate-400">
                              ...
                              <span className="font-semibold text-slate-700">
                                {formatRupiah(calculatedHargaJualSaran)}
                              </span>
                              <button
                                type="button"
          ...
          <div className="flex items-center justify-between pt-2 border-t border-slate-50">
            ...
            <Button
              ...
              disabled={isFinalizing}
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs h-9 px-4 rounded-xl border-none cursor-pointer flex items-center gap-1.5 shadow-xs"
            >
