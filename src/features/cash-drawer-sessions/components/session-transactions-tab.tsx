            ...
            {sortedTransactions.length > 0 ? (
                <div className="border border-slate-100 dark:border-slate-850 rounded-xl overflow-x-auto bg-white dark:bg-slate-950 shadow-2xs">
                    <table className="w-full min-w-[500px] border-collapse text-left text-xs">
                        <thead>
                            <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-855 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider text-[9px] sm:text-[10px]">
                                <th className="py-2.5 px-3 sm:py-3 sm:px-4">No. Transaksi</th>
                                <th className="py-2.5 px-3 sm:py-3 sm:px-4">Waktu</th>
                                <th className="py-2.5 px-3 sm:py-3 sm:px-4 text-center">Metode</th>
                                <th className="py-2.5 px-3 sm:py-3 sm:px-4 text-center">Item</th>
                                    ...
                                    >
                                        <td className="py-2.5 px-3 sm:py-3 sm:px-4">
                                            <div className="flex flex-col gap-0.5">
                                                <span className={cn(
                                                    "font-bold font-mono tracking-tight text-slate-900 dark:text-slate-100 text-[11px] sm:text-xs",
                                        ...
                                        <td className="py-2.5 px-3 sm:py-3 sm:px-4 text-slate-505 dark:text-slate-450 font-semibold text-[11px] sm:text-xs">
                                            ...
                                            <TooltipProvider delayDuration={150}>
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <button className="inline-flex items-center gap-1 text-[10px] sm:text-[11px] font-bold text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200 border-none bg-transparent cursor-help focus:outline-none">
                                                            <span>{itemsCount} Item</span>
                                                            <IconInfoCircle size={11} className="text-slate-400 shrink-0" />
                                                        </button>
                                                    </TooltipTrigger>
                                        ...
                                        <td className="py-2.5 px-3 sm:py-3 sm:px-4 text-right">
                                            <div className="flex flex-col items-end">
                                                <span className={cn(
                                                    "font-extrabold text-slate-900 dark:text-slate-100 tabular-nums text-[11px] sm:text-xs",
